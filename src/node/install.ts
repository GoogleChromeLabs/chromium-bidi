...
export async function downloadBrowser(): Promise<void> {
  ...
  function fetchBinary() {
    ...
    if (revisionInfo.local) {
      logPolitely(
        `chrome is already in ${revisionInfo.folderPath}; skipping download.`
      );
      return;
    }
    ...
    function onError(error: Error) {
      console.error(
        `ERROR: Failed to set up chrome r${revisionInfo.revision}!`
      );
      console.error(error);
      process.exit(1);
    }
    ...
    function onProgress(downloadedBytes: number, totalBytes: number) {
      if (!progressBar) {
        progressBar = new ProgressBar(
          `Downloading ${'chrome'} r${revisionInfo.revision} - ${toMegabytes(
            totalBytes
          )} [:bar] :percent :etas `,
          {
            complete: '=',
