...
export class NetworkRequest {
  ...
  #getResponseStartedEvent(): Network.ResponseStarted {
    assert(this.#request.info, 'RequestWillBeSentEvent is not set');
    assert(
      this.#response.extraInfo || this.#response.info,
      'None of ResponseReceivedExtraInfoEvent or ResponseReceivedEvent are set'
    );
    ...
    const headers = bidiNetworkHeadersFromCdpNetworkHeaders(
      this.#response.extraInfo?.headers ?? this.#response.info?.headers
    );
    ...
    return {
      ...
      params: {
        ...
        response: {
          ...
          bytesReceived: this.#response.info?.encodedDataLength ?? -1,
