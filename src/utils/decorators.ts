...
type Consumer = {
  getters: Map<symbol | (abstract new (...args: any[]) => any), () => any>;
  fields: Map<string | symbol, object>;
};

type Provider = Map<
  symbol | (abstract new (...args: any[]) => any),
  ((value: any) => void)[]
>;
 ...
 */
export function feed(marks: InjectableKey[]) {
  return (
    target: any,
    ...
    context.addInitializer(function initializer() {
      ...
      for (const mark of marks) {
        ...
        setters.push(reinject.bind(undefined, consumers, mark));
  ...
  >(
    ...
    context.addInitializer(function initializer() {
      ...
      switch (context.kind) {
        ...
        case 'accessor': {
          const setters = provider.get(mark) ?? [];
          setters.push(context.access.set.bind(undefined, this));
          provider.set(mark, setters);
          break;
        }
        default:
