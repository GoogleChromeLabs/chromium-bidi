        ...
        for (const definition of node.body) {
          if (definition.type === 'PropertyDefinition') {
            for (const decorator of definition.decorators) {
              for (const name of ['pantry', 'feed']) {
                if (decorator.expression.callee.name === name) {
                  fields.set(definition.key.name, [name, definition]);
                }
              }
            }
          }
        ...
        for (const decorator of node.decorators) {
          for (const name of ['pantry', 'feed']) {
            if (decorator.expression.callee.name === name) {
              context.report({
                node: decorator,
                message: `Cannot decorate uninitialized field with '${name}'. Either initialize the field or use 'accessor'.`,
              });
            }
          }
        }
