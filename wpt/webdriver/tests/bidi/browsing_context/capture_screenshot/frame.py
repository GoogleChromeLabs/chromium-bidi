...
from tests.support.screenshot import (DEFAULT_CONTENT, REFERENCE_CONTENT,
                                      REFERENCE_STYLE, OUTER_IFRAME_STYLE,
                                      INNER_IFRAME_STYLE)
...
@pytest.mark.asyncio
async def test_context_origin(bidi_session, top_context, inline, iframe,
                              compare_png_bidi, domain):
    initial_url = inline(f"{REFERENCE_STYLE}{REFERENCE_CONTENT}")
    await bidi_session.browsing_context.navigate(
        context=top_context["context"], url=initial_url, wait="complete")
    expected_size = await viewport_dimensions(bidi_session, top_context)
    reference_data = await bidi_session.browsing_context.capture_screenshot(
        context=top_context["context"])
    assert png_dimensions(reference_data) == expected_size
    ...
    iframe_content = f"{INNER_IFRAME_STYLE}{DEFAULT_CONTENT}"
    new_url = inline(
        f"{OUTER_IFRAME_STYLE}{iframe(iframe_content, domain=domain)}")
    await bidi_session.browsing_context.navigate(
        context=top_context["context"], url=new_url, wait="complete")
